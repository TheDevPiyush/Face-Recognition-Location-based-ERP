...
class AttendanceRecordView(APIView):
    ...
    def post(self, request):
        """Create or update (upsert) attendance record for a given user and attendance window.

        Body fields:
        - attendance_window: int (required)
        - user: int (optional; required for teacher/admin when marking for a student)
        """
        if allowed := check_allow_roles(
            request.user, [User.Role.TEACHER, User.Role.ADMIN, User.Role.STUDENT]
        ):
        ...
        if request.user.role == User.Role.STUDENT:
            ...
            if not body_user_id:
                return Response(
                    {"message": "'user' is required for teacher/admin"},
                    status=status.HTTP_400_BAD_REQUEST,
                )
            target_user = get_object_or_404(User, pk=body_user_id)
        ...
        if not target_user.batch_id or target_user.batch_id != (
            window.target_batch_id or None
        ):
            return Response(
                {"message": "User does not belong to the window's batch"},
                status=status.HTTP_400_BAD_REQUEST,
